/*
 * Code generated by Speakeasy (https://speakeasyapi.dev). DO NOT EDIT.
 */

import * as components from "../components";
import * as z from "zod";

/**
 * The name of the Flow.
 */
export enum ListFlowOutcomesPathParamFlow {
    Checkout = "checkout",
    CardTransaction = "card-transaction",
    NonCardTransaction = "non-card-transaction",
    RedirectTransaction = "redirect-transaction",
}

/**
 * Action for the given rule. Actions can only be used in
 *
 * @remarks
 * flows that support them.
 *
 * * The `checkout` flow only supports the
 * `select-payment-options` action.
 * * The `card-transaction` supports the `route-transaction`,
 * `skip-3ds`, and `decline-early` actions.
 * * The `non-card-transaction` flow only supports the
 * `decline-early` action.
 * * The `redirect-transaction` flow only supports
 * the `route-transaction` action.
 *
 */
export enum ListFlowOutcomesPathParamAction {
    SelectPaymentOptions = "select-payment-options",
    DeclineEarly = "decline-early",
    RouteTransaction = "route-transaction",
    Skip3ds = "skip-3ds",
}

export type ListFlowOutcomesRequest = {
    /**
     * The flow name. This can be one of the following.
     *
     * @remarks
     *
     * - `checkout` - Applies during checkout to determine what payment options are shown.
     * - `card-transaction` - Applies when processing a card transaction.
     * - `non-card-transaction` - Applies when processing a gift card only transaction, or a
     * redirect transaction using the `decline-early` action.
     * - `redirect-transaction` - Applies when processing any other transaction.
     */
    flow: ListFlowOutcomesPathParamFlow;
    /**
     * The flow action.
     */
    action: ListFlowOutcomesPathParamAction;
    /**
     * An ISO 639-1 Language Code and optional ISO 3166 Country Code. This locale
     *
     * @remarks
     * determines the language for the labels returned for every payment option.
     */
    locale?: string | undefined;
};

export type ListFlowOutcomesResponse = {
    httpMeta: components.HTTPMetadata;
    /**
     * Returns a list of rule outcomes.
     */
    flowRuleOutcomes?: components.FlowRuleOutcomes | undefined;
};

/** @internal */
export const ListFlowOutcomesPathParamFlow$ = z.nativeEnum(ListFlowOutcomesPathParamFlow);

/** @internal */
export const ListFlowOutcomesPathParamAction$ = z.nativeEnum(ListFlowOutcomesPathParamAction);

/** @internal */
export namespace ListFlowOutcomesRequest$ {
    export type Inbound = {
        flow: ListFlowOutcomesPathParamFlow;
        action: ListFlowOutcomesPathParamAction;
        locale?: string | undefined;
    };

    export const inboundSchema: z.ZodType<ListFlowOutcomesRequest, z.ZodTypeDef, Inbound> = z
        .object({
            flow: ListFlowOutcomesPathParamFlow$,
            action: ListFlowOutcomesPathParamAction$,
            locale: z.string().default("en"),
        })
        .transform((v) => {
            return {
                flow: v.flow,
                action: v.action,
                locale: v.locale,
            };
        });

    export type Outbound = {
        flow: ListFlowOutcomesPathParamFlow;
        action: ListFlowOutcomesPathParamAction;
        locale: string;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, ListFlowOutcomesRequest> = z
        .object({
            flow: ListFlowOutcomesPathParamFlow$,
            action: ListFlowOutcomesPathParamAction$,
            locale: z.string().default("en"),
        })
        .transform((v) => {
            return {
                flow: v.flow,
                action: v.action,
                locale: v.locale,
            };
        });
}

/** @internal */
export namespace ListFlowOutcomesResponse$ {
    export type Inbound = {
        HttpMeta: components.HTTPMetadata$.Inbound;
        FlowRuleOutcomes?: components.FlowRuleOutcomes$.Inbound | undefined;
    };

    export const inboundSchema: z.ZodType<ListFlowOutcomesResponse, z.ZodTypeDef, Inbound> = z
        .object({
            HttpMeta: components.HTTPMetadata$.inboundSchema,
            FlowRuleOutcomes: components.FlowRuleOutcomes$.inboundSchema.optional(),
        })
        .transform((v) => {
            return {
                httpMeta: v.HttpMeta,
                ...(v.FlowRuleOutcomes === undefined
                    ? null
                    : { flowRuleOutcomes: v.FlowRuleOutcomes }),
            };
        });

    export type Outbound = {
        HttpMeta: components.HTTPMetadata$.Outbound;
        FlowRuleOutcomes?: components.FlowRuleOutcomes$.Outbound | undefined;
    };

    export const outboundSchema: z.ZodType<Outbound, z.ZodTypeDef, ListFlowOutcomesResponse> = z
        .object({
            httpMeta: components.HTTPMetadata$.outboundSchema,
            flowRuleOutcomes: components.FlowRuleOutcomes$.outboundSchema.optional(),
        })
        .transform((v) => {
            return {
                HttpMeta: v.httpMeta,
                ...(v.flowRuleOutcomes === undefined
                    ? null
                    : { FlowRuleOutcomes: v.flowRuleOutcomes }),
            };
        });
}
