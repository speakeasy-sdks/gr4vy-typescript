 ...
 */
export function accountUpdaterJobsCreate(
  client: Gr4vyCore,
...
> {
  const input: operations.CreateAccountUpdaterJobRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) =>
      operations.CreateAccountUpdaterJobRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.AccountUpdaterJobCreate, {
    explode: true,
  });

  const path = pathToFunc("/account-updater/jobs")();
  ...
  const [result] = await M.match<
    ...
    | ConnectionError
  >(
    M.json(201, components.AccountUpdaterJob$inboundSchema.optional()),
