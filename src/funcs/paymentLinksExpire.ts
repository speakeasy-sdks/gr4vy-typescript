 ...
 */
export function paymentLinksExpire(
  client: Gr4vyCore,
...
> {
  const input: operations.ExpirePaymentLinkRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) => operations.ExpirePaymentLinkRequest$outboundSchema.parse(value),
  ...
  const pathParams = {
    payment_link_id: encodeSimple("payment_link_id", payload.payment_link_id, {
      explode: false,
      charEncoding: "percent",
    }),
  };

  const path = pathToFunc("/payment-links/{payment_link_id}/expire")(
    pathParams,
  );
