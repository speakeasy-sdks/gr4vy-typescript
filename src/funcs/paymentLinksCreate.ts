...
> {
  const input: operations.AddPaymentLinkRequest = {
  ...
  const parsed = safeParse(
    input,
    (value) => operations.AddPaymentLinkRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.PaymentLinkCreate, { explode: true });
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(201, components.PaymentLink$inboundSchema),
